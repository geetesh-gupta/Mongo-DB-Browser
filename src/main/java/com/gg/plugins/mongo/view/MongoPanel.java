...
public class MongoPanel extends JPanel implements Disposable {
	...
	private MongoResultPanel createResultPanel(Project project, Notifier notifier) {
		return new MongoResultPanel(project, new MongoDocumentOperations() {
			...
			public Document getMongoDocument(Object _id) {
				return mongoService.findMongoDocument(configuration,
						navigation.getCurrentWayPoint().getCollection(),
						_id);
			}

			public void deleteMongoDocument(Object objectId) {
				mongoService.delete(configuration, navigation.getCurrentWayPoint().getCollection(), objectId);
				executeQuery();
			}

			public void updateMongoDocument(Document mongoDocument) {
				mongoService.update(configuration, navigation.getCurrentWayPoint().getCollection(), mongoDocument);
				executeQuery();
			}
			...
			public Document getReferenceDocument(String collection, Object _id, String database) {
				return mongoService.findMongoDocument(configuration,
						new MongoCollection(collection,
								database != null ?
								new MongoDatabase(database, new MongoServer(configuration)) :
	...
	private void executeQuery(final boolean useCachedResults, final Navigation.WayPoint wayPoint) {
		...
		ProgressManager.getInstance()
		               .run(new Task.Backgroundable(project, "Get documents from " + wayPoint.getLabel()) {
			               @Override
			               public void run(@NotNull ProgressIndicator indicator) {
				               try {
					               ...
					               if (!useCachedResults) {
						               currentResults = mongoService.findMongoDocuments(configuration,
					               ...
					               UIUtil.invokeLaterIfNeeded(() -> {
						               ...
						               rowCountLabel.setText(String.format("%s documents",
								               currentResults.getDocuments().size()));
